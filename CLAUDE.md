...
- অ্যাডমিন বেল নোটিফিকেশন (`NotificationComposer`) — `notifReprints`; গোনা হয় `copy_no > 1` **এবং** নিজের নয় এমন প্রিন্ট, `store_config.reprint_alert_seen_at` এর পরের; `printLogs()` খুললেই seen সময় আপডেট হয় (শেষ রেকর্ডের `printed_at`, `now()` নয় — চলমান প্রিন্ট যেন না-দেখা থাকে)
- ⚠️ seen সময়টা শপ-ভিত্তিক, ইউজার-ভিত্তিক নয় — এক অ্যাডমিন দেখলে সব অ্যাডমিনের ব্যাজ সরে যায়
