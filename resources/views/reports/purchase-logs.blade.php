...
function showDetail(id) {
    ...
    if (before) {
        ...
        fields.forEach(([key, label, fmt]) => {
            ...
            // Compare formatted output, not raw values — a DB decimal column
            // round-trips as a string ("16500.00") before a save and a PHP
            // float (16500.0) right after ->update() in the same request, so
            // JSON.stringify would flag it as "changed" even when it isn't.
