@extends('layouts.app')
@section('title', 'খরচের ক্যাটাগরি')
@section('page-title', 'খরচের ক্যাটাগরি')

@section('content')
@include('partials.page-header', ['title' => 'অতিরিক্ত খরচের ক্যাটাগরি', 'createRoute' => null])
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;align-items:start">
    ...
    {{-- Add form --}}
    <div class="card">
        <div class="card-header"><h3><i class="fas fa-plus-circle" style="color:var(--accent)"></i> নতুন ক্যাটাগরি যোগ করুন</h3></div>
        <div style="padding:20px">
            <form method="POST" action="{{ route('extra-cost-categories.store') }}">
                @csrf
                <div class="form-group-field" style="margin-bottom:14px">
                    <label>ক্যাটাগরির নাম <span class="req">*</span></label>
                    <input type="text" name="name" placeholder="যেমন: পরিবহন, প্যাকেজিং, লোডিং…"
            ...
            <table class="data-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <form method="POST" action="{{ route('extra-cost-categories.update', $cat) }}"
                                  style="display:flex;gap:6px;align-items:center" id="edit-form-{{ $cat->id }}">
                                @csrf @method('PUT')
                        ...
                        <td class="tc">
                            {{-- edit trigger already inline above --}}
                        </td>
                        <td class="tc">
                            <form method="POST" action="{{ route('extra-cost-categories.destroy', $cat) }}"
                    ...
                    @empty
                    <tr><td colspan="4" class="empty-row">কোনো ক্যাটাগরি নেই — বাম থেকে যোগ করুন</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
