@extends('layouts.app')
@section('title', 'কাস্টমার লেজার রিপোর্ট')
...
<div class="ledger-select-wrap">
    ...
    <div class="ledger-select-card">
        ...
        <form id="ledgerForm" onsubmit="goToLedger(event)" class="ledger-select-form">
            ...
            {{-- Customer search --}}
            <div class="ls-field">
                <label class="ls-label">কাস্টমার</label>
                <div class="ls-search-wrap">
                    <i class="fas fa-search ls-search-icon"></i>
                    <input type="text" id="customerSearch" class="ls-input" placeholder="নাম, প্রোপ্রাইটর বা ফোন দিয়ে খুঁজুন..."
                        autocomplete="off" oninput="filterCustomers()">
                </div>
                <div id="customerDropdown" class="ls-dropdown" style="display:none"></div>
                <input type="hidden" id="customerId" name="customer_id">
                <div id="selectedCustomer" class="ls-selected" style="display:none">
                    <i class="fas fa-user-check" style="color:#16a34a;margin-right:6px"></i>
                    <span id="selectedName"></span>
                    <button type="button" onclick="clearCustomer()" class="ls-clear-btn">
    ...
    <div class="ls-recent-card">
        <div class="ls-recent-title">
            <i class="fas fa-users"></i>
            <span id="panelTitle">সকল কাস্টমার</span>
            <span id="panelCount" class="ls-panel-count">{{ count($customers) }}</span>
        </div>
        <div class="ls-recent-list" id="customerPanel">
            @foreach($customers as $c)
            <button type="button" class="ls-recent-item"
                data-area="{{ $c->area_id ?? '' }}"
                onclick="pickCustomer({{ $c->id }}, '{{ addslashes($c->name) }}')">
                <div class="ls-recent-name">{{ $c->name }}</div>
                @if($c->area)
                <div class="ls-recent-sub" style="color:#7c3aed;font-size:.72rem">
                    <i class="fas fa-map-marker-alt" style="font-size:.65rem"></i> {{ $c->area->name }}
                </div>
                @endif
                @if($c->proprietor)
                <div class="ls-recent-sub">প্রোঃ {{ $c->proprietor }}</div>
                @endif
                @if($c->due_amount > 0)
...
.ls-select:focus { outline: none; border-color: var(--accent); }
.ls-select-icon {
...
function activeCustomers() {
    ...
    return allCustomers.filter(c => String(c.area_id) === selectedArea);
}

function updatePanel() {
    const areaId = selectedArea;
    const items  = document.querySelectorAll('#customerPanel .ls-recent-item');
    let visible  = 0;
    items.forEach(el => {
        const show = !areaId || el.dataset.area === areaId;
...
function filterCustomers() {
    ...
    const pool = activeCustomers();   // already filtered by area
    ...
    if (!matches.length) { dd.innerHTML = '<div class="ls-dropdown-item" style="color:#94a3b8">কোনো কাস্টমার পাওয়া যায়নি</div>'; dd.style.display = 'block'; return; }

    dd.innerHTML = matches.map(c => `
        <div class="ls-dropdown-item" onclick="pickCustomer(${c.id}, '${c.name.replace(/'/g,"\\'")}')">
            <div class="ls-dropdown-name">${c.name}</div>
...
function goToLedger(e) {
    ...
    window.location.href = `/customers/${selectedId}/ledger?from=${from}&to=${to}`;
}
